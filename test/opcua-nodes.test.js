      ...
      browse: sinon.stub().resolves([
        {
          ...
          nodeId: { toString: () => "i=85" },
          nodeClass: "Object",
          typeDefinition: { toString: () => "i=61" },
          isForward: true,
        },
        {
          browseName: { name: "Types" },
          nodeId: { toString: () => "i=86" },
          nodeClass: "Object",
          typeDefinition: { toString: () => "i=61" },
          isForward: true,
        },
...
describe("opcua-method node", function () {
  ...
  it("should call method with configured nodeIds", async function () {
    ...
    const msg = {
      inputArguments: [
        { dataType: "Double", value: 3 },
        { dataType: "Double", value: 7 },
      ],
    };
    const send = sinon.stub();
    const done = sinon.stub();
  ...
  it("should override nodeIds from msg", async function () {
    ...
    methodCtor.call(node, {
      id: "m2",
      endpoint: "ep1",
      objectNodeId: "ns=1;s=Default",
      methodNodeId: "ns=1;s=Default",
    });

    const msg = {
      objectNodeId: "ns=1;s=Methods",
      methodNodeId: "ns=1;s=Methods.Multiply",
      inputArguments: [],
    };
  ...
  it("should show kein Endpoint status when no endpoint", function () {
    ...
    expect(node.status.calledWith(sinon.match({ text: "no endpoint" }))).to.be
      .true;
  });
});
...
describe("opcua-event node", function () {
  ...
  it("should set connected status when endpoint is connected", function () {
    ...
    expect(node.status.calledWith(sinon.match({ text: "connected" }))).to.be
      .true;
  });
...
describe("opcua-server node", function () {
      ...
      require.cache[opcuaPath] = {
        ...
        exports: {
          OPCUAServer: function (opts) {
            ...
            this.getEndpointUrl = () =>
              `opc.tcp://test:${opts.port}/UA/NodeRED`;
            this.engine = {
              addressSpace: { getOwnNamespace: () => ({ index: 1 }) },
            };
          },
          Variant: function (o) {
  ...
  describe("addVariable/addMethod parent reference (issues #12, #16)", function () {
    ...
    it("uses organizedBy when adding a variable to the standard ObjectsFolder", async function () {
      ...
      expect(capturedAddVariable).to.have.property(
        "organizedBy",
        "ObjectsFolder",
      );
