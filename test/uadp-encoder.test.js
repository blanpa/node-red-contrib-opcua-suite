...
function roundTripNm(nm) {
  ...
  if (Array.isArray(buf))
    throw new Error(
      "expected single Buffer, got Array — use roundTripNmChunked",
    );
...
describe("uadp-encoder — DataSetMessage", function () {
  describe("encodeDataSetMessage / decodeDataSetMessage stubs replaced", function () {
    it("encodeDataSetMessage returns a Buffer (not a stub throw)", function () {
      const buf = encodeDataSetMessage({
  ...
  describe("DataSetFlags1/2 flag cascade", function () {
    it("default keyframe emits no DataSetFlags2 byte (bit 7 of DSF1 = 0)", function () {
      ...
      const dsf1 = buf[0];
      expect(dsf1 & 0x80).to.equal(
        0,
        "DataSetFlags2 must not be present for default keyframe",
      );
    });
    ...
    it("configurationVersion.major sets DSF1 bit 5", function () {
      const buf = encodeDataSetMessage({
        fieldEncoding: "variant",
        messageType: "keyframe",
        configurationVersion: { major: 1 },
        fields: {},
      });
      expect(buf[0] & 0x20).to.equal(
        0x20,
        "configVersionMajor bit must be set",
      );
    });
    ...
    it("configurationVersion.minor sets DSF1 bit 6", function () {
      ...
      expect(buf[0] & 0x40).to.equal(
        0x40,
        "configVersionMinor bit must be set",
      );
    });
    ...
    it("deltaframe messageType triggers DSF2 presence (bit 0 of DSF2 = 1)", function () {
      ...
      expect(buf[0] & 0x80).to.equal(
        0x80,
        "DSF1 bit 7 must be set (DSF2 present)",
      );
      expect(buf[1] & 0x0f).to.equal(
        0b0001,
        "DSF2 bits 0-3 must be 0001 for deltaframe",
      );
    });
    ...
    it("keepalive messageType triggers DSF2 bits 0-3 = 0011", function () {
      ...
      expect(buf[0] & 0x80).to.equal(
        0x80,
        "DSF1 bit 7 must be set for keepalive (DSF2 present)",
      );
      expect(buf[1] & 0x0f).to.equal(
        0b0011,
        "DSF2 bits 0-3 must be 0011 for keepalive",
      );
    });
    ...
    it("event messageType triggers DSF2 bits 0-3 = 0010", function () {
      ...
      expect(buf[0] & 0x80).to.equal(
        0x80,
        "DSF1 bit 7 must be set for event (DSF2 present)",
      );
      expect(buf[1] & 0x0f).to.equal(
        0b0010,
        "DSF2 bits 0-3 must be 0010 for event",
      );
    });
    ...
    it("picoseconds sets DSF2 bit 5 (when timestamp also present)", function () {
      ...
      expect(buf[1] & 0x20).to.equal(
        0x20,
        "picoseconds bit must be set in DSF2",
      );
    });
    ...
    it("picoseconds is NOT set when timestamp is absent", function () {
      ...
      if (buf[0] & 0x80) {
        expect(buf[1] & 0x20).to.equal(
          0,
          "picoseconds bit must not be set without timestamp",
        );
      }
    });
  });
  ...
  describe("round-trip: variant fieldEncoding", function () {
    it("round-trips Double scalar", function () {
      const dsm = {
        ...
        messageType: "keyframe",
        sequenceNumber: 1,
        fields: { temp: { dataType: "Double", value: 23.5 } },
      };
    ...
    it("round-trips Boolean scalar", function () {
      const dsm = {
        ...
        messageType: "keyframe",
        sequenceNumber: 1,
        fields: { ok: { dataType: "Boolean", value: true } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips UInt32 scalar", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { count: { dataType: "UInt32", value: 100 } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips UInt16 scalar", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { id: { dataType: "UInt16", value: 65535 } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips Int32 scalar", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { diff: { dataType: "Int32", value: -42 } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips String scalar", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { name: { dataType: "String", value: "hello world" } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips Float scalar", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { f: { dataType: "Float", value: 3.14 } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips Byte scalar", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { b: { dataType: "Byte", value: 255 } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips Int16 scalar", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { s: { dataType: "Int16", value: -1000 } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips SByte scalar", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { sb: { dataType: "SByte", value: -128 } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips UInt64 BigInt scalar", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { bigval: { dataType: "UInt64", value: 9007199254740993n } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips StatusCode scalar", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { sc: { dataType: "StatusCode", value: 0x80350000 } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips DateTime scalar", function () {
      ...
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { t: { dataType: "DateTime", value: ts } },
      };
      const d = roundTripDsm(dsm);
    ...
    it("round-trips UInt32 array Variant", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { arr: { dataType: "UInt32", value: [1, 2, 3] } },
      };
      const d = roundTripDsm(dsm);
  ...
  describe("round-trip: datavalue fieldEncoding", function () {
    ...
    it("round-trips datavalue with statusCode", function () {
      const dsm = {
        ...
        fields: {
          s: {
            value: { dataType: "UInt32", value: 42 },
            statusCode: 0x80350000,
          },
        },
      };
      const d = roundTripDsm(dsm);
  ...
  describe("error handling", function () {
    it("throws UADP_ENCODE_INVALID_INPUT on null input to encodeDataSetMessage", function () {
      expect(() => encodeDataSetMessage(null)).to.throw(
        /UADP_ENCODE_INVALID_INPUT/,
      );
    });

    it("throws UADP_ENCODE_INVALID_FIELD_ENCODING on unknown fieldEncoding", function () {
      expect(() =>
        encodeDataSetMessage({ fieldEncoding: "unknown", fields: {} }),
      ).to.throw(/UADP_ENCODE_INVALID_FIELD_ENCODING/);
    });
    ...
    it("throws UADP_ENCODE_INVALID_MESSAGE_TYPE on unknown messageType", function () {
      ...
      ).to.throw(/UADP_ENCODE_INVALID_MESSAGE_TYPE/);
    });
    ...
    it("throws UADP_VARIANT_UNSUPPORTED_BUILTIN_TYPE on unknown dataType id", function () {
      const dsm = {
        fieldEncoding: "variant",
        messageType: "keyframe",
        fields: { bad: { dataType: 99, value: 0 } },
      };
      expect(() => encodeDataSetMessage(dsm)).to.throw(
        /UADP_VARIANT_UNSUPPORTED_BUILTIN_TYPE/,
      );
    });
    ...
    it("throws UADP_RAWDATA_DECODE_REQUIRES_METADATA when decoding rawdata", function () {
      const dsm = {
        fieldEncoding: "rawdata",
        messageType: "keyframe",
        fields: { x: { dataType: "UInt32", value: 5 } },
      };
      const buf = encodeDataSetMessage(dsm);
      expect(() => decodeDataSetMessage(buf)).to.throw(
        /UADP_RAWDATA_DECODE_REQUIRES_METADATA/,
      );
    });
  });
});
        ...
        payload: [
          {
            fieldEncoding: "variant",
            messageType: "keyframe",
            fields: { v: { dataType: "Boolean", value: false } },
          },
        ],
      };
        ...
        payload: [
          {
            fieldEncoding: "variant",
            messageType: "keyframe",
            fields: { a: { dataType: "UInt32", value: 1 } },
          },
          {
            fieldEncoding: "variant",
            messageType: "keyframe",
            fields: { b: { dataType: "String", value: "hello" } },
          },
        ],
      };
      const buf = encodeNetworkMessage(nm);
  ...
  describe("chunking", function () {
    it("produces Array<Buffer> when payload exceeds default MTU (1400 bytes)", function () {
      ...
      expect(Array.isArray(result)).to.equal(
        true,
        "expected Array<Buffer> for oversized payload",
      );
      expect(result.length).to.be.greaterThanOrEqual(
        2,
    ...
    it("each chunk is <= MTU (1400 bytes) with default MTU", function () {
      ...
      for (const chunk of result) {
        expect(chunk.length).to.be.at.most(
          1400,
          `chunk of length ${chunk.length} exceeds MTU`,
        );
      }
    });
    ...
    it("total chunk data sums to full payload size (TotalSize consistency)", function () {
      ...
      for (const chunk of result) {
        ...
        if (lastTotalSize === null) lastTotalSize = d.chunk.totalSize;
          ...
          expect(d.chunk.totalSize).to.equal(
            lastTotalSize,
            "TotalSize must be same across all chunks",
          );
        sumChunkData += d.chunk.chunkData.length;
      }
      expect(sumChunkData).to.equal(
        lastTotalSize,
        "sum of chunkData lengths must equal TotalSize",
      );
    });
    ...
    it("each chunk has ExtendedFlags2 bit 0 set (chunk marker)", function () {
      ...
      for (const chunk of result) {
        ...
        expect(extFlags2 & 0x01).to.equal(
          0x01,
          "ExtFlags2 bit 0 (chunk) must be set",
        );
      }
    });
    ...
    it("produces a single Buffer when payload is small enough (below MTU)", function () {
      ...
      const result = encodeNetworkMessage(nm);
      expect(Buffer.isBuffer(result)).to.equal(
        true,
        "small payload should return a single Buffer",
      );
    });
    ...
    it("respects custom mtu via opts", function () {
      ...
      expect(Array.isArray(result)).to.equal(
        true,
        "expected chunking with tiny mtu=50",
      );
      for (const chunk of result) {
        expect(chunk.length).to.be.at.most(
          50,
          `chunk exceeds custom mtu: ${chunk.length}`,
        );
      }
    });
  });
    ...
    describe(name, function () {
      it("first byte (UADPFlags) matches fixture.flags.uadpFlags", function () {
        ...
        if (vec.isStaticChunk) {
          ...
          expect(buf[0] & 0x80).to.equal(
            0x80,
            "chunk messages must have ExtFlags1 enabled",
          );
          return;
        }
      ...
      it("decodeNetworkMessage(hex) round-trips key fields from fixture.model", function () {
        ...
        if (vec.model.timestamp) {
          expect(decoded.timestamp.getTime()).to.equal(
            vec.model.timestamp.getTime(),
          );
        }
        if (vec.model.dataSetClassId) {
          expect(decoded.dataSetClassId.toUpperCase()).to.equal(
            vec.model.dataSetClassId.toUpperCase(),
          );
        }
      });
    });
...
describe("uadp-encoder — PublisherId variants (all 5 types, ExtFlags1 type bits)", function () {
  ...
  for (const c of cases) {
    it(`${c.name} (typeBits=0b${c.typeBits.toString(2).padStart(3, "0")}): PublisherId presence + round-trip`, function () {
      ...
      expect(buf[0] & 0x10).to.equal(
        0x10,
        "UADPFlags bit 4 (PublisherId) must be set",
      );
      if (c.typeBits === 0b000) {
        ...
        expect(buf[0] & 0x80).to.equal(
          0x00,
          "Byte publisherId: ExtFlags1 suppressed (all bits zero), UADPFlags bit 7 must be 0",
        );
      } else {
        // Non-Byte types trigger ExtFlags1 (type bits are non-zero)
        expect(buf[0] & 0x80).to.equal(
          0x80,
          "UADPFlags bit 7 (ExtFlags1) must be set for non-Byte publisherId",
        );
        // ExtFlags1 bits 0-2 must equal the expected type bits
        expect(buf[1] & 0x07).to.equal(
          c.typeBits,
          `ExtFlags1 bits 0-2 must be 0b${c.typeBits.toString(2)} for ${c.name}`,
        );
      }
...
describe("uadp-encoder — chunking (fixture assertions, Part 14 §7.2.4.4.4)", function () {
  it("input larger than MTU returns Array<Buffer>; each chunk <= MTU; sum of chunk data == totalSize", function () {
    ...
    expect(Array.isArray(result)).to.equal(
      true,
      "large payload must return Array<Buffer>",
    );
    ...
    for (const chunk of result) {
      expect(chunk.length).to.be.at.most(
        1400,
        "each chunk must be <= 1400 bytes (default MTU)",
      );
      const d = decodeNetworkMessage(chunk);
      expect(d.chunk).to.exist;
      if (totalSize === null) totalSize = d.chunk.totalSize;
        ...
        expect(d.chunk.totalSize).to.equal(
          totalSize,
          "totalSize must be consistent across chunks",
        );
      totalChunkData += d.chunk.chunkData.length;
    }
    expect(totalChunkData).to.equal(
      totalSize,
      "sum of chunkData lengths must equal totalSize",
    );
  });
...
describe("uadp-encoder — decoder error handling (T-02-01, T-02-05, T-02-08)", function () {
  it("truncated buffer throws UADP_DECODE_TRUNCATED with offset info", function () {
    ...
    let caught = null;
    try {
      decodeNetworkMessage(Buffer.from([0x91]));
    } catch (e) {
      caught = e;
    }
  ...
  it("non-Buffer input throws UADP_DECODE_INVALID_INPUT", function () {
    expect(() => decodeNetworkMessage("not a buffer")).to.throw(
      /UADP_DECODE_INVALID_INPUT/,
    );
  });
  ...
  it("unsupported UADP version (version bits != 1) throws UADP_DECODE_UNSUPPORTED_VERSION", function () {
    ...
    expect(() => decodeNetworkMessage(Buffer.from([0x02]))).to.throw(
      /UADP_DECODE_UNSUPPORTED_VERSION/,
    );
  });

  it("null input throws UADP_ENCODE_INVALID_INPUT on encodeNetworkMessage", function () {
    expect(() => encodeNetworkMessage(null)).to.throw(
      /UADP_ENCODE_INVALID_INPUT/,
    );
  });
});
