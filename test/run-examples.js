...
function loadAndPatchExamples() {
  ...
  for (const file of examples) {
    ...
    // that need a broker.
    const pubsubConns = flows.filter(
      (n) => n.type === "opcua-pubsub-connection",
    );
...
async function main() {
  ...
  if (settings.status !== 200) {
    console.error(
      red(
        `   Cannot reach Node-RED at ${NODE_RED_URL} (status ${settings.status})`,
      ),
    );
    process.exit(2);
  }
  console.log(
    green(`   ✓ Node-RED ${settings.data?.version || "reachable"}\n`),
  );

  console.log(dim("2. Loading and patching example flows..."));
  const { allFlows, meta } = loadAndPatchExamples();
  console.log(
    green(`   ✓ ${meta.length} examples, ${allFlows.length} total nodes\n`),
  );
  ...
  if (dep.status !== 204 && dep.status !== 200) {
    console.error(
      red(`   ✗ Deploy failed: HTTP ${dep.status} ${JSON.stringify(dep.data)}`),
    );
    process.exit(3);
  }
  ...
  for (const ex of meta) {
    ...
    if (ex.transports && ex.transports.length) {
      ...
      if (ex.transports.includes("mqtt")) {
        const broker =
          (ex.brokerUrls && ex.brokerUrls[0]) || "mqtt://localhost:1883";
        console.log(
          dim(
            `  note: requires an MQTT broker at ${broker} (e.g. docker run -p 1883:1883 eclipse-mosquitto)`,
          ),
        );
      }
    }
    ...
    for (const inj of ex.injects) {
      ...
      const errMsgs = newMsgs.filter((m) => {
        ...
        return (
          f.toLowerCase().includes("error") || s.toLowerCase().includes("error")
        );
      });
      ...
      if (!httpOK) {
        ...
        status = "WARN";
        totalFail++;
        const sample = (errMsgs[0].msg || errMsgs[0].format || "")
          .toString()
          .slice(0, 100);
        label = yellow(
          `  ⚠ ${inj.name || inj.id} — debug shows error: ${sample}`,
        );
        ...
        status = "OK";
        totalPass++;
        const sample = (newMsgs[0].msg || newMsgs[0].format || "")
          .toString()
          .slice(0, 60);
        label = green(`  ✓ ${inj.name || inj.id} — ${sample}`);
      }
      console.log(label);
      exResults.injects.push({
        name: inj.name,
        id: inj.id,
        status,
        debugCount: newMsgs.length,
      });
    }
  ...
  console.log(cyan("────────────────────────────────────────────────"));
  console.log(
    `  Result: ${green(totalPass + " OK")}, ${red(totalFail + " Errors/Warnings")}`,
  );
  console.log(cyan("────────────────────────────────────────────────"));
