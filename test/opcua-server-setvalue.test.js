...
describe('opcua-server setValue datatype resolution', function () {
    ...
    const cases = [
        { name: 'Boolean', datatype: 'Boolean', value: true, expect: DataType.Boolean },
        { name: 'Int32', datatype: 'Int32', value: 4711, expect: DataType.Int32 },
        { name: 'String', datatype: 'String', value: 'hello', expect: DataType.String },
        { name: 'Double', datatype: 'Double', value: 25.5, expect: DataType.Double }
    ];
    ...
    for (const c of cases) {
        it(`setValue without msg.datatype works on a ${c.name} variable`, async function () {
            ...
            const set = await invoke({ command: 'setValue', nodeId, payload: c.value });
    ...
    it('an explicit msg.datatype still wins', async function () {
        ...
        await invoke({ command: 'addVariable', variableName: 'SV_Explicit', datatype: 'Int32', nodeId });
        const set = await invoke({ command: 'setValue', nodeId, datatype: 'Int32', payload: 7 });
    ...
    it('addVariable defaults a Byte variable to 0, not null', async function () {
        ...
        const add = await invoke({
            command: 'addVariable', variableName: 'SV_Byte', datatype: 'Byte', nodeId
        });
        expect(add.err).to.equal(null);
        const dv = await readValue(nodeId);
    ...
    it('addVariable defaults an SByte variable to 0, not null', async function () {
        ...
        const add = await invoke({
            command: 'addVariable', variableName: 'SV_SByte', datatype: 'SByte', nodeId
        });
        expect(add.err).to.equal(null);
        const dv = await readValue(nodeId);
    ...
    it('rejects msg.func when allowMsgFunc is not enabled', async function () {
        const obj = await invoke({ command: 'addObject', objectName: 'GateTestObj' });
    ...
    it('addMethod without msg.func still works with allowMsgFunc off', async function () {
        const obj = await invoke({ command: 'addObject', objectName: 'GateTestObj2' });
        const res = await invoke({
            command: 'addMethod',
            methodName: 'Ungated',
            parentNodeId: obj.msg.nodeId,
