...
const PUBSUB_FLOWS = [
  {
    file: "10 - PubSub UDP-UADP Loopback.json",
    transport: "udp",
    encoding: "uadp",
  },
...
describe("example flows (static validation)", function () {
  it("the examples directory is non-empty and includes the fourteen bundled flows", function () {
    ...
    for (let n = 1; n <= 14; n++) {
      ...
      expect(
        found,
        `expected an example flow starting with "${prefix}"`,
      ).to.equal(true);
    }
  });
          ...
          if (Array.isArray(n.wires)) {
            n.wires.forEach(function (port) {
              (port || []).forEach(function (target) {
                expect(
                  ids.has(target),
                  `${n.id} wires -> missing node ${target}`,
                ).to.equal(true);
              });
            });
          }
  ...
  PUBSUB_FLOWS.forEach(function (spec) {
    describe(`PubSub flow ${spec.file}`, function () {
      ...
      it("contains one connection, one publisher, one subscriber, an inject and a debug", function () {
        expect(
          flow.filter((n) => n.type === "opcua-pubsub-connection").length,
        ).to.equal(1);
        expect(
          flow.filter((n) => n.type === "opcua-publisher").length,
        ).to.equal(1);
        expect(
          flow.filter((n) => n.type === "opcua-subscriber").length,
        ).to.equal(1);
        expect(flow.filter((n) => n.type === "inject").length).to.be.at.least(
          1,
        );
  ...
  describe("PubSub flow 13 - PubSub Full Validation.json", function () {
    ...
    it("is a multi-tab suite: a README tab plus one tab per scenario T1..T9 (>= 10 tabs)", function () {
      ...
      for (let n = 1; n <= 9; n++) {
        expect(labels, `missing a tab for T${n}`).to.match(
          new RegExp("T" + n + "\\b"),
        );
      }
    });
    ...
    it("exercises both transports and both encodings across its connections", function () {
      ...
      expect(transports.has("mqtt"), "expected an MQTT connection").to.equal(
        true,
      );
      const encodings = new Set(
        flow
          .filter((n) => n.type === "opcua-publisher")
          .map((p) => p.messageEncoding),
      );
    ...
    it("every publisher/subscriber resolves its connection and encoding matches the connected publisher", function () {
      const connById = new Map(
        flow
          .filter((n) => n.type === "opcua-pubsub-connection")
          .map((c) => [c.id, c]),
      );
      flow.forEach(function (n) {
        if (n.type === "opcua-publisher" || n.type === "opcua-subscriber") {
          expect(
            connById.has(n.connection),
            `${n.id} -> missing connection ${n.connection}`,
          ).to.equal(true);
        }
      });
    });
    ...
    it("MQTT connections target the validation broker mqtt://val-mosquitto:1883", function () {
      flow
        .filter(
          (n) =>
            n.type === "opcua-pubsub-connection" && n.transportType === "mqtt",
        )
        .forEach(function (c) {
    ...
    it("exercises a cyclic publisher (KeepAlive scenario) and a multi-writer publisher", function () {
      ...
      expect(
        pubs.some((p) => p.publishMode === "cyclic"),
        "expected a cyclic publisher (T5)",
      ).to.equal(true);
      ...
      expect(
        multiWriter,
        "expected a publisher with >= 2 DataSetWriters (T4)",
      ).to.equal(true);
    });
    ...
    it("exercises chunking via a small maxNetworkMessageSize publisher (T6)", function () {
      const chunked = flow.some(
        (n) =>
          n.type === "opcua-publisher" &&
          Number(n.maxNetworkMessageSize) <= 300,
      );
      expect(
        chunked,
        "expected a small-MTU publisher to force chunking",
      ).to.equal(true);
    });
    ...
    it("exercises a ConfigurationVersion-mismatch subscriber (T9)", function () {
      const cvSub = flow.some(
        (n) =>
          n.type === "opcua-subscriber" &&
          n.expectedConfigVersion &&
          n.expectedConfigVersion !== "",
      );
      expect(
        cvSub,
        "expected a subscriber with expectedConfigVersion (T9)",
      ).to.equal(true);
    });
  });
  ...
  describe("Full Suite flow 14 - Full Suite Validation.json", function () {
    ...
    it("classic client tabs share a None-security endpoint and an auth endpoint exists", function () {
      ...
      const noneEp = endpoints.find(
        (e) => e.securityMode === "None" && e.securityPolicy === "None",
      );
      expect(noneEp, "expected a None/None security endpoint").to.be.an(
        "object",
      );
    ...
    exampleFiles.forEach(function (file) {
      ...
      const udpConnIds = new Set(
        flow
          .filter(
            (n) =>
              n.type === "opcua-pubsub-connection" && n.transportType === "udp",
          )
          .map((n) => n.id),
      );
      flow.forEach(function (n) {
