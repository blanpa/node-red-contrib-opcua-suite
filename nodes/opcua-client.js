...
module.exports = function(RED) {
    function OpcUaClientNode(config) {
        ...
        if (!endpointConfig.getSharedManager) {
            node.error('Endpoint node does not support connection sharing — please update');
            return;
        }
        ...
        node.on('input', async function(msg, send, done) {
            try {
                ...
                const operation = (msg.operation || config.defaultOperation || 'read').toLowerCase();
                let result;
                ...
                switch (operation) {
                    ...
                    case 'subscribe':
                        await handleSubscribe(msg, clientManager, send, node, subscription, monitorItems,
        ...
        node.on('close', async function(removed, done) {
            for (const monitorItem of monitorItems.values()) {
                try { await monitorItem.terminate(); } catch (e) { /* ignore */ }
            }
            monitorItems.clear();
    ...
    async function handleReadMultiple(msg, mgr) {
        ...
        let items = msg.items;
        if ((!items || !Array.isArray(items) || items.length === 0) && msg.payload) {
            if (Array.isArray(msg.payload) && msg.payload.length > 0 && msg.payload[0].nodeId) {
                items = msg.payload;
        ...
        const nodeIds = items.map((item, idx) => {
            ...
            if (!nodeId) throw new Error(`Item[${idx}]: Invalid NodeId: ${nodeIdStr}`);
            return nodeId;
        });
    ...
    async function handleWrite(msg, mgr) {
        ...
        const result = await mgr.write(nodeId, value, msg.datatype || null);
        return {
            payload: value,
            statusCode: result.statusCode,
            nodeId: nodeIdString
        ...
        if ((!items || !Array.isArray(items) || items.length === 0) && msg.payload) {
            if (Array.isArray(msg.payload) && msg.payload.length > 0 && msg.payload[0].nodeId) {
                ...
                items = Object.entries(msg.payload).map(([nodeId, valObj]) => {
                    if (typeof valObj === 'object' && valObj !== null && valObj.value !== undefined) {
                        return { nodeId, value: valObj.value, datatype: valObj.datatype || null, name: valObj.name };
                    }
    ...
    async function handleSubscribe(msg, mgr, send, node, subscription, monitorItems, setSubscription) {
        const nodeIdString = msg.topic || msg.nodeId;
        ...
        if (!subscription) {
            subscription = await mgr.createSubscription({ interval, maxNotificationsPerPublish: queueSize });
            setSubscription(subscription);
        }

        if (monitorItems.has(nodeIdString)) {
            const resultMsg = { ...msg, payload: 'Already subscribed', nodeId: nodeIdString };
            send(resultMsg);
            return;
        }
    ...
    async function handleMethod(msg, mgr) {
        ...
        const result = await mgr.callMethod(objParsed, methParsed, Array.isArray(args) ? args : [args]);
    ...
    async function handleHistory(msg, mgr) {
        ...
        const startTime = msg.startTime || msg.payload?.startTime || new Date(Date.now() - 3600000);
    ...
    async function handleReadAttribute(msg, mgr) {
        ...
        return { payload: result.value, statusCode: result.statusCode, nodeId: nodeIdString };
    }
