...
class UdpTransport extends BaseTransport {
  ...
  send(payload, _opts) {
    ...
    for (const chunk of chunks) {
      this._socket.send(
        ...
        (err) => {
          if (err)
            this.emit("error", createError(`UDP_SEND_ERROR: ${err.message}`));
        },
      );
    }
  }
  ...
  _onDatagram(buf, rinfo) {
    ...
    const writerGroupId = partial.groupHeader
      ? partial.groupHeader.writerGroupId
      : undefined;
    ...
    const mtu =
      this.config && typeof this.config.mtu === "number"
        ? this.config.mtu
        : 1400;
    ...
    if (
      !this._chunks.has(key) &&
      this._chunks.size >= MAX_INFLIGHT_REASSEMBLIES
    ) {
      const oldestKey = this._chunks.keys().next().value; // Map insertion order
      this._chunks.delete(oldestKey);
      this.emit(
        "warn",
        createError(`UDP_REASSEMBLY_OVERFLOW: dropped oldest key ${oldestKey}`),
      );
    }
