...
class MqttTransport extends BaseTransport {
  ...
  _tryConnect(protocolVersion) {
    return new Promise((resolve, reject) => {
      ...
      client.on("connect", () => {
        ...
        // Subscribe to the data-topic subtree so inbound NetworkMessages are
        // delivered to 'message' listeners (subscriber nodes). Without this the
        // broker forwards nothing and the receive path is dead. The topic scheme
        // is `${topicPrefix}/${publisherId}/${writerGroupId}/${dataSetWriterId}`
        // (see _buildTopic); subscribing to `${prefix}/#` receives every frame
        // and software DataSetReader filtering (publisherId/writerGroupId/
        // dataSetWriterId) selects the relevant ones. Subscribe failures are
        // surfaced as 'error' but do not block 'connected'.
        // LO-05: subscribing to the whole `${prefix}/#` subtree means QoS cost
        // and bandwidth scale with ALL traffic published under the prefix, not
        // just the writers this node cares about (software filtering happens
        // after delivery).
        const prefix = this.config.topicPrefix || "ua";
        client.subscribe(`${prefix}/#`, { qos: this.config.qos != null ? this.config.qos : 1 }, (err) => {
          if (err) this.emit("error", createError(`MQTT_SUBSCRIBE_ERROR: ${err.message}`));
        });
        this.emit("connected");
        resolve();
      });
  ...
  /**
   * Builds the data-topic from prefix + identifiers, validating every
   * user-controlled component against TOPIC_FORBIDDEN BEFORE concatenation
   * (T-03-04). Throws TOPIC_INVALID_CHARACTER synchronously on any violation.
   * @param {object} opts - { writerGroupId, dataSetWriterId }
   * @returns {string} `${topicPrefix}/${publisherId}/${writerGroupId}/${dataSetWriterId}`
   */
  _buildTopic(opts) {
    const prefix = this.config.topicPrefix || "ua";
  ...
  /**
   * Publishes one or more buffers to the data-topic.
   *
   *   - retain:false is HARDCODED (T-03-06, Part 14 §7.3.4) — opts.retain is
   *     NEVER read, so a caller cannot poison new subscribers with a stale
   *     retained message.
   *   - Topic is validated against injection BEFORE publish (T-03-04); a bad
   *     identifier throws synchronously and nothing is published.
   *   - qos resolution: opts.qos > config.qos > 1 (D-15).
   *   - Buffer | Buffer[] dispatched via Array.isArray; one publish per chunk.
   *   - Not connected → emits 'error' (MQTT_SEND_NOT_CONNECTED), never throws.
   *   - Broker publish failure → 'error' (MQTT_PUBLISH_ERROR).
   *
   * @param {Buffer|Buffer[]} payload
   * @param {object} [opts] - { writerGroupId, dataSetWriterId, qos? } (retain ignored).
   * @returns {void}
   */
  send(payload, opts) {
    if (!this._client) {
    ...
    // Throws TOPIC_INVALID_CHARACTER synchronously on injection / missing id so
    // the caller cannot accidentally publish to a wildcarded/malformed topic.
    const topic = this._buildTopic(opts);

    const qos =
      opts && opts.qos != null
        ? opts.qos
        : this.config.qos != null
          ? this.config.qos
          : 1;

    const publishOpts = {
      qos,
      // T-03-06: HARDCODED literal on a fresh object. opts.retain is never read,
      // copied, or spread — caller-override is structurally impossible.
      retain: false,
    };
