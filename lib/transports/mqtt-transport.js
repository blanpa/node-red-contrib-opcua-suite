...
class MqttTransport extends BaseTransport {
  ...
  _tryConnect(protocolVersion) {
    return new Promise((resolve, reject) => {
      const opts = {
        protocolVersion,
        reconnectPeriod:
          this.config.reconnectPeriod != null
      ...
      client.on("connect", () => {
        ...
        client.subscribe(
          `${prefix}/#`,
          { qos: this.config.qos != null ? this.config.qos : 1 },
          (err) => {
            if (err)
              this.emit(
                "error",
                createError(`MQTT_SUBSCRIBE_ERROR: ${err.message}`),
              );
          },
        );
        this.emit("connected");
  ...
  send(payload, opts) {
    if (!this._client) {
      this.emit(
        "error",
        createError("MQTT_SEND_NOT_CONNECTED: client is null"),
      );
      return;
    }
