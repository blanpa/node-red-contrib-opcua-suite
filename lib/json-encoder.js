...
const MESSAGE_TYPE_MODEL = Object.fromEntries(
  Object.entries(MESSAGE_TYPE_WIRE).map(([k, v]) => [v, k]),
);
...
function _convertValueForJson(value, dataType) {
  ...
  if (
    ...
    "namespaceIndex" in value
  ) {
    return nodeIdToString(value);
  }
  // Array — convert recursively
  if (Array.isArray(value))
    return value.map((v) => _convertValueForJson(v, dataType));
...
function _convertValueFromJson(value, uaType) {
  if (value === null || value === undefined) return null;
  switch (uaType) {
    case UA_TYPE.DateTime:
      return new Date(value);
    case UA_TYPE.ByteString:
    ...
    default:
      if (Array.isArray(value))
        return value.map((v) => _convertValueFromJson(v, uaType));
      return value;
  }
}
...
function _decodeVariant(obj, path) {
  ...
  if (typeof obj.UaType !== "number") {
    ...
    err.code = "JSON_DECODE_INVALID_TYPE";
    err.path = `${path}.UaType`;
    throw err;
  }
  return {
    dataType: obj.UaType,
    value: _convertValueFromJson(obj.Value, obj.UaType),
  };
}
...
function _encodeDataSetMessage(dsm) {
  const parts = [];
  if (dsm.dataSetWriterId !== undefined)
    parts.push(`"DataSetWriterId":${JSON.stringify(dsm.dataSetWriterId)}`);
  if (dsm.dataSetWriterName !== undefined)
    parts.push(`"DataSetWriterName":${JSON.stringify(dsm.dataSetWriterName)}`);
  if (dsm.publisherId !== undefined)
    parts.push(`"PublisherId":${JSON.stringify(String(dsm.publisherId))}`);
  if (dsm.writerGroupName !== undefined)
    parts.push(`"WriterGroupName":${JSON.stringify(dsm.writerGroupName)}`);
  if (dsm.sequenceNumber !== undefined)
    parts.push(`"SequenceNumber":${JSON.stringify(dsm.sequenceNumber)}`);
  if (dsm.configurationVersion !== undefined) {
    const cv = dsm.configurationVersion;
    parts.push(
      `"MetaDataVersion":{"MajorVersion":${JSON.stringify(cv.major)},"MinorVersion":${JSON.stringify(cv.minor)}}`,
    );
  }
  if (dsm.timestamp instanceof Date)
    parts.push(`"Timestamp":${JSON.stringify(dsm.timestamp.toISOString())}`);
  if (dsm.status !== undefined)
    parts.push(`"Status":${JSON.stringify(dsm.status)}`);

  const messageTypeWire = MESSAGE_TYPE_WIRE[dsm.messageType || "keyframe"];
  if (!messageTypeWire)
    throw createError(`JSON_ENCODE_INVALID_MESSAGE_TYPE: ${dsm.messageType}`);
...
function _decodeDataSetMessage(obj, path) {
  if (!obj || typeof obj !== "object") {
    const err = createError(
      `Required DataSetMessage object missing at '${path}'`,
    );
    err.code = "JSON_DECODE_MISSING_FIELD";
    err.path = path;
    throw err;
  ...
  const dsm = {};
  if (obj.DataSetWriterId !== undefined)
    dsm.dataSetWriterId = obj.DataSetWriterId;
  if (obj.DataSetWriterName !== undefined)
    dsm.dataSetWriterName = obj.DataSetWriterName;
  if (obj.PublisherId !== undefined) dsm.publisherId = obj.PublisherId;
  if (obj.WriterGroupName !== undefined)
    dsm.writerGroupName = obj.WriterGroupName;
  if (obj.SequenceNumber !== undefined) dsm.sequenceNumber = obj.SequenceNumber;
  if (obj.MetaDataVersion)
    dsm.configurationVersion = {
      major: obj.MetaDataVersion.MajorVersion,
      minor: obj.MetaDataVersion.MinorVersion,
    };
...
function encodeNetworkMessage(networkMessage, _opts) {
  if (!networkMessage || typeof networkMessage !== "object") {
    throw createError(
      "JSON_ENCODE_INVALID_INPUT: networkMessage must be an object",
    );
  }
  ...
  // PublisherId — preserve the source JS type so it round-trips identically to the
  // UADP path (HI-02): a numeric id stays a JSON number, a string stays a string,
  // a BigInt (UInt64) is emitted as a decimal string (safe JSON, like Variant).
  if (networkMessage.publisherId !== undefined) {
    const pid = networkMessage.publisherId;
    const pidJson =
      typeof pid === "bigint"
        ? JSON.stringify(pid.toString())
        : JSON.stringify(pid);
    parts.push(`"PublisherId":${pidJson}`);
  }
  if (networkMessage.writerGroupName !== undefined)
    parts.push(
      `"WriterGroupName":${JSON.stringify(networkMessage.writerGroupName)}`,
    );
  if (networkMessage.dataSetClassId)
    parts.push(
      `"DataSetClassId":${JSON.stringify(networkMessage.dataSetClassId)}`,
    );

  // Group header (CR-02) — carry the writer-group identity and the NM-level
  // SequenceNumber so a writerGroupId-filtered subscriber matches and msg.sequenceNumber
  // is the NM sequence on JSON just as it is on UADP. Per Part 14 §7.2.5 these are
  // group-header network-message header fields; emitted flat as WriterGroupId /
  // SequenceNumber per the encoder's hard-coded field-order convention.
  const groupHeader = networkMessage.groupHeader;
  if (groupHeader && groupHeader.writerGroupId !== undefined) {
    parts.push(`"WriterGroupId":${JSON.stringify(groupHeader.writerGroupId)}`);
  }
  if (groupHeader && groupHeader.sequenceNumber !== undefined) {
    parts.push(
      `"SequenceNumber":${JSON.stringify(groupHeader.sequenceNumber)}`,
    );
  }
  // NM-level Timestamp (HI-01) — publish time, ISO-8601 per Part 14 §7.2.5, so the
  // subscriber emits msg.timestamp as publish time (matching UADP) not receipt time.
  if (networkMessage.timestamp instanceof Date) {
    parts.push(
      `"Timestamp":${JSON.stringify(networkMessage.timestamp.toISOString())}`,
    );
  }

  const dsms = Array.isArray(networkMessage.payload)
...
function decodeNetworkMessage(jsonString, _opts) {
  ...
  try {
    ...
    err.code = "JSON_DECODE_PARSE_ERROR";
    err.path = "";
    throw err;
  }
  if (!obj || typeof obj !== "object") {
  ...
  // PublisherId round-trips with its native JSON type (HI-02): a number stays a
  // number, a string stays a string — matching the UADP decoder's behavior.
  if (obj.PublisherId !== undefined) nm.publisherId = obj.PublisherId;
  if (obj.WriterGroupName !== undefined)
    nm.writerGroupName = obj.WriterGroupName;
  if (obj.DataSetClassId) nm.dataSetClassId = obj.DataSetClassId;
  // Reconstruct the group header (CR-02) so writerGroupId filtering works and the
  // subscriber reads the NM-level sequenceNumber, identical to the UADP path.
  if (obj.WriterGroupId !== undefined || obj.SequenceNumber !== undefined) {
    nm.groupHeader = {};
    if (obj.WriterGroupId !== undefined)
      nm.groupHeader.writerGroupId = obj.WriterGroupId;
    if (obj.SequenceNumber !== undefined)
      nm.groupHeader.sequenceNumber = obj.SequenceNumber;
  }
  // NM-level timestamp (HI-01) — publish time, decoded back to a Date.
  if (obj.Timestamp) nm.timestamp = new Date(obj.Timestamp);
  nm.payload = obj.Messages.map((dsm, i) =>
    _decodeDataSetMessage(dsm, `Messages[${i}]`),
  );
  return nm;
}
