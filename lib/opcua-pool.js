...
class PooledClientManager extends EventEmitter {
  ...
  // arrayType is the 5th parameter of OpcUaClientManager.write(); omitting it
  // here silently dropped array writes down to scalars whenever poolSize > 1.
  write(nodeId, value, datatype, dataTypeNodeId, arrayType) {
    return this._pickMember().write(
      nodeId,
      value,
      datatype,
      dataTypeNodeId,
  ...
  browse(nodeId, options) {
    return this._pickMember().browse(nodeId, options);
  }
