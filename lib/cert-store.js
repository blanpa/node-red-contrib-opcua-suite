...
function registerCertRoutes(RED, prefix, certsDir) {
  ...
  RED.httpAdmin.post(prefix + "/upload-cert", needsWrite, async function (req, res) {
    try {
      const data = req.body || {};
  ...
  RED.httpAdmin.delete(prefix + "/upload-cert/:filename", needsWrite, async function (req, res) {
    try {
