...
function parseNodeId(nodeIdString) {
  ...
  try {
    ...
    //
    // Only the FIRST ";" separates the namespace from the identifier — a
    // String identifier may legitimately contain further semicolons
    // (e.g. "ns=2;s=Line1;Motor"). Splitting on every ";" and taking
    // parts[1] truncated such NodeIds to "Line1".
    if (nodeIdString.includes(";")) {
      const sep = nodeIdString.indexOf(";");
      const head = nodeIdString.substring(0, sep);
      let namespaceIndex = 0;
      let identifierPart = nodeIdString;

      if (head.startsWith("ns=")) {
        namespaceIndex = parseInt(head.substring(3), 10);
        if (!Number.isFinite(namespaceIndex)) return null;
        identifierPart = nodeIdString.substring(sep + 1);
      }
...
/**
 * Creates a real Error in Node-RED format (ME-01).
 *
 * Historically this returned a PLAIN object ({ message, error, stack }), which
 * broke `instanceof Error`, Error.captureStackTrace and Node-RED's `done(err)`
 * (LO-01). It now returns a genuine Error instance while preserving every
 * property current callers rely on:
 *   - .message : the outer message (native Error field)
 *   - .stack   : a real stack captured at this call site (native Error field)
 *   - .error   : the inner error's message string (back-compat for callers that
 *                read err.error and for msg.error consumers in nodes/*.js)
 *   - .cause   : the original Error, when one was passed (standard chaining)
 *
 * Callers in lib/pubsub-config.js and lib/json-encoder.js attach extra fields
 * (.code, .errors, .path) AFTER creation and then `throw` — those continue to
 * work unchanged because Object.assign / direct assignment work on a real Error.
 *
 * @param {string} message - the outer error message
 * @param {Error|null} [error] - an optional inner/original error to wrap
 * @returns {Error}
 */
function createError(message, error = null) {
  const e = new Error(message);
  // Back-compat: callers historically read err.error as the inner message.
  e.error = error ? error.message : undefined;
  if (error) {
    e.cause = error;
  }
  return e;
 ...
 */
// Guards against pathological/self-referential value graphs. OPC UA structures
// are trees in practice, but this is decoded network input — an unbounded
// recursion here would take the whole Node-RED process down with a stack
// overflow rather than yielding a bad message.
const MAX_SERIALIZE_DEPTH = 32;

function serializeExtensionObject(extObj, depth = 0, seen = null) {
  ...
  if (Array.isArray(extObj)) {
    return extObj.map((item) => serializeExtensionObject(item, depth + 1, seen));
  }
  ...
  for (const key of keys) {
    ...
    if (val === null || val === undefined) {
      ...
      result[key] = serializeExtensionObject(val, depth + 1, seen);
    } else {
      result[key] = val;
    }
  }
