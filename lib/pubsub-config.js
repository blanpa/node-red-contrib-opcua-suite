...
const ABSTRACT_TYPES_FOR_RAWDATA = new Set([
  "NodeId",
  "ExpandedNodeId",
  "DiagnosticInfo",
]);
...
function validateWriterGroup(cfg) {
  ...
  if (!cfg || typeof cfg !== "object") {
    return {
      valid: false,
      errors: [_issue("", "MUST_BE_OBJECT", "cfg must be an object")],
    };
  }
  ...
  if (cfg.keepAliveTime !== undefined) {
    if (!_isPositiveNumber(cfg.keepAliveTime)) {
      ...
      _isPositiveNumber(cfg.publishingInterval) &&
      cfg.keepAliveTime < cfg.publishingInterval
    ) {
      errors.push(
        _issue(
          "keepAliveTime",
          "MUST_BE_GTE_PUBLISHING_INTERVAL",
  ...
  if (
    cfg.maxNetworkMessageSize !== undefined &&
    !_isPositiveNumber(cfg.maxNetworkMessageSize)
  ) {
    errors.push(
      _issue(
  ...
  if (
    ...
    errors.push(
      _issue(
        "priority",
        "MUST_BE_BYTE_RANGE",
        "priority must be integer 0..255",
      ),
    );
  }
...
function WriterGroup(cfg) {
  ...
  return Object.freeze({
    ...
    maxNetworkMessageSize:
      cfg.maxNetworkMessageSize ?? DEFAULT_MAX_NETWORK_MESSAGE_SIZE,
...
function validatePublishedDataSet(cfg) {
  ...
  if (!Array.isArray(cfg.fields) || cfg.fields.length === 0) {
    ...
    cfg.fields.forEach(function (f, i) {
      if (!f || typeof f !== "object") {
        errors.push(
          _issue(`fields[${i}]`, "MUST_BE_OBJECT", "field must be an object"),
        );
        return;
      }
      ...
      if (typeof f.dataType !== "string") {
        errors.push(
          _issue(
            `fields[${i}].dataType`,
            "MUST_BE_STRING",
            "field.dataType required as string",
          ),
        );
      }
    });
  ...
  if (cfg.configurationVersion !== undefined) {
    ...
    if (
      !cv ||
      typeof cv !== "object" ||
      !Number.isInteger(cv.major) ||
      !Number.isInteger(cv.minor)
    ) {
      errors.push(
        _issue(
          "configurationVersion",
          "MUST_BE_VERSION_PAIR",
...
function PublishedDataSet(cfg) {
  ...
  return Object.freeze({
    ...
    configurationVersion: Object.freeze(
      cfg.configurationVersion ?? { ...DEFAULT_PUBLISHED_DS_VERSION },
    ),
  });
}
...
function validateDataSetWriter(cfg) {
  ...
  if (cfg.dataSetName !== undefined && typeof cfg.dataSetName !== "string") {
    errors.push(
      _issue("dataSetName", "MUST_BE_STRING", "dataSetName must be a string"),
    );
  }
  // keyFrameCount: optional integer >= 0
  if (
    cfg.keyFrameCount !== undefined &&
    !_isNonNegativeInt(cfg.keyFrameCount)
  ) {
    errors.push(
      _issue(
        "keyFrameCount",
        "MUST_BE_NONNEGATIVE_INTEGER",
  ...
  if (
    cfg.dataSetFieldContentMask !== undefined &&
    !Number.isInteger(cfg.dataSetFieldContentMask)
  ) {
    errors.push(
      _issue(
  ...
  if (mask & FIELD_CONTENT_MASK_RAWDATA_BIT) {
    if (!cfg.publishedDataSet || !Array.isArray(cfg.publishedDataSet.fields)) {
      errors.push(
        _issue(
          "publishedDataSet",
          "RAW_DATA_REQUIRES_PUBLISHED_DS",
      ...
      cfg.publishedDataSet.fields.forEach(function (f, i) {
        if (ABSTRACT_TYPES_FOR_RAWDATA.has(f.dataType)) {
          errors.push(
            _issue(
              `publishedDataSet.fields[${i}].dataType`,
        ...
        if (
          f.dataType === "String" &&
          (f.maxStringLength === undefined ||
            !_isNonNegativeInt(f.maxStringLength))
        ) {
          errors.push(
            _issue(
              `publishedDataSet.fields[${i}].maxStringLength`,
...
function DataSetWriter(cfg) {
  ...
  return Object.freeze({
    ...
    publishedDataSet: cfg.publishedDataSet
      ? PublishedDataSet(cfg.publishedDataSet)
      : undefined,
  });
}
...
function validateDataSetReader(cfg) {
  ...
  if (
    cfg.keepAliveTime !== undefined &&
    !_isPositiveNumber(cfg.keepAliveTime)
  ) {
    errors.push(
      _issue(
        "keepAliveTime",
  ...
  if (
    cfg.messageReceiveTimeout !== undefined &&
    !_isPositiveNumber(cfg.messageReceiveTimeout)
  ) {
    errors.push(
      _issue(
...
function DataSetReader(cfg) {
  ...
  const keepAlive = _isPositiveNumber(cfg.keepAliveTime)
    ? cfg.keepAliveTime
    : 0;
  const defaultTimeout = Math.max(
    DEFAULT_RECEIVE_TIMEOUT_FACTOR * keepAlive,
    DEFAULT_RECEIVE_TIMEOUT_MIN_MS,
  );
  return Object.freeze({
