...
class BinaryStream {
  ...
  _ensureRead(n) {
    if (this._cursor + n > this._buf.length) {
      throw createError(
        `UADP_DECODE_TRUNCATED at offset ${this._cursor} (need ${n} bytes, have ${this._buf.length - this._cursor})`,
      );
    }
  }
  ...
  _ensureWrite(n) {
    if (this._cursor + n > this._buf.length) {
      ...
      const grown = Buffer.allocUnsafe(
        Math.max(this._buf.length * 2, this._cursor + n),
      );
  ...
  writeUInt8(v) {
    this._ensureWrite(1);
    this._buf.writeUInt8(v, this._cursor);
    this._cursor += 1;
  }
  writeUInt16LE(v) {
    this._ensureWrite(2);
    this._buf.writeUInt16LE(v, this._cursor);
    this._cursor += 2;
  }
  writeUInt32LE(v) {
    this._ensureWrite(4);
    this._buf.writeUInt32LE(v, this._cursor);
    this._cursor += 4;
  }
  writeUInt64LE(v) {
    this._ensureWrite(8);
    this._buf.writeBigUInt64LE(
  ...
  writeGuid(uuidStr) {
    ...
    if (parts.length !== 5)
      throw createError(`UADP_ENCODE_INVALID_GUID: ${uuidStr}`);
  ...
  readUInt8() {
    this._ensureRead(1);
    const v = this._buf.readUInt8(this._cursor);
    this._cursor += 1;
    return v;
  }
  readUInt16LE() {
    this._ensureRead(2);
    const v = this._buf.readUInt16LE(this._cursor);
    this._cursor += 2;
    return v;
  }
  readUInt32LE() {
    this._ensureRead(4);
    const v = this._buf.readUInt32LE(this._cursor);
    this._cursor += 4;
    return v;
  }
  readUInt64LE() {
    this._ensureRead(8);
    const v = this._buf.readBigUInt64LE(this._cursor);
    this._cursor += 8;
    return v;
  }
  ...
  readDateTime() {
    ...
    const ms =
      Number(ticks / DATETIME_TICKS_PER_MS) - Number(FILETIME_EPOCH_OFFSET_MS);
    return new Date(ms);
  }
   ...
   */
  toBuffer() {
    return this._buf.subarray(0, this._cursor);
  }

  get remaining() {
    return this._buf.length - this._cursor;
  }
  get position() {
    return this._cursor;
  }
...
function _writeVariantScalar(stream, builtIn, value) {
  switch (builtIn) {
    ...
    case BUILTIN_TYPE.SByte:
      stream._ensureWrite(1);
      stream._buf.writeInt8(value, stream._cursor);
      stream._cursor += 1;
      break;
    ...
    case BUILTIN_TYPE.Int16:
      stream._ensureWrite(2);
      stream._buf.writeInt16LE(value, stream._cursor);
      stream._cursor += 2;
      break;
    ...
    case BUILTIN_TYPE.Int32:
      stream._ensureWrite(4);
      stream._buf.writeInt32LE(value, stream._cursor);
      stream._cursor += 4;
      break;
    ...
    case BUILTIN_TYPE.Float:
      stream._ensureWrite(4);
      stream._buf.writeFloatLE(value, stream._cursor);
      stream._cursor += 4;
      break;
    case BUILTIN_TYPE.Double:
      stream._ensureWrite(8);
      stream._buf.writeDoubleLE(value, stream._cursor);
      stream._cursor += 8;
      break;
    ...
    case BUILTIN_TYPE.ByteString:
      ...
      stream.writeUInt32LE(value.length);
...
function _writeVariant(stream, variant) {
  ...
  const builtIn =
    typeof dataType === "number" ? dataType : BUILTIN_TYPE[dataType];
  ...
  if (isArray) {
    ...
    stream._ensureWrite(4);
    stream._buf.writeInt32LE(value.length, stream._cursor);
    stream._cursor += 4;
...
function _readVariant(stream) {
  ...
  if (isArray) {
    ...
    value = [];
    for (let i = 0; i < len; i++)
      value.push(_readVariantScalar(stream, builtIn));
  } else {
...
function _publisherIdTypeBits(value) {
  ...
  if (typeof value === "number") {
    if (!Number.isInteger(value) || value < 0) {
      throw createError(
        `UADP_ENCODE_INVALID_PUBLISHER_ID: must be non-negative integer, got ${value}`,
      );
    }
    ...
    throw createError(
      `UADP_ENCODE_PUBLISHER_ID_TOO_LARGE: use BigInt for UInt64, got ${value}`,
    );
  }
  throw createError(
    `UADP_ENCODE_PUBLISHER_ID_TYPE: unsupported type ${typeof value}`,
  );
}

function _writePublisherId(stream, value, typeBits) {
  switch (typeBits) {
    case PUBLISHER_ID_TYPE.BYTE:
      stream.writeUInt8(value);
      break;
    case PUBLISHER_ID_TYPE.UINT16:
      stream.writeUInt16LE(value);
      break;
    case PUBLISHER_ID_TYPE.UINT32:
      stream.writeUInt32LE(value);
      break;
    case PUBLISHER_ID_TYPE.UINT64:
      stream.writeUInt64LE(value);
      break;
    case PUBLISHER_ID_TYPE.STRING:
      stream.writeString(value);
      break;
  }
}
...
function _readPublisherId(stream, typeBits) {
  switch (typeBits) {
    ...
    case PUBLISHER_ID_TYPE.UINT32:
      return stream.readUInt32LE();
    case PUBLISHER_ID_TYPE.UINT64:
      return stream.readUInt64LE(); // returns BigInt
    case PUBLISHER_ID_TYPE.STRING:
      return stream.readString();
    default:
      throw createError(`UADP_DECODE_INVALID_PUBLISHER_ID_TYPE: ${typeBits}`);
  }
}
...
function _buildExtendedFlags1(nm, extFlags2) {
  let f = 0x00;
  if (nm.publisherId !== undefined)
    f |= _publisherIdTypeBits(nm.publisherId) & 0x07; // bits 0-2
 ...
 */
function _writeNetworkMessageHeaderFields(
  stream,
  nm,
  uadpFlags,
  extFlags1,
...
function _splitIntoChunks(fullPayload, mtu, headerOverhead) {
  ...
  if (maxChunkData <= 0) {
    throw createError(
      `UADP_CHUNK_MTU_TOO_SMALL: mtu ${mtu} is too small to fit chunk header overhead ${headerOverhead + CHUNK_PAYLOAD_OVERHEAD}`,
    );
  }
  const chunks = [];
  let offset = 0;
 ...
 * @returns {Buffer}
 */
function _encodeChunkNetworkMessage(
  originalNm,
  chunkInfo,
  ...
  _writeNetworkMessageHeaderFields(
    stream,
    chunkNm,
    uadpFlags,
    extFlags1,
...
function encodeDataSetMessage(dsm, _opts) {
  if (!dsm || typeof dsm !== "object") {
    throw createError(
      "UADP_ENCODE_INVALID_INPUT: dataSetMessage must be an object",
    );
  }
  ...
  if (encodingBits === undefined)
    throw createError(`UADP_ENCODE_INVALID_FIELD_ENCODING: ${fieldEncoding}`);
  if (messageBits === undefined)
    throw createError(`UADP_ENCODE_INVALID_MESSAGE_TYPE: ${messageType}`);
      ...
      if (fieldEncoding === "variant") {
        ...
        const builtIn =
          typeof field.dataType === "number"
            ? field.dataType
            : BUILTIN_TYPE[field.dataType];
        if (!builtIn)
          throw createError(
            `UADP_RAWDATA_UNKNOWN_TYPE on field "${name}": ${field.dataType}`,
          );
...
function decodeDataSetMessage(buffer, _opts) {
  ...
  if (dsm.fieldEncoding === "rawdata") {
    throw createError(
      "UADP_RAWDATA_DECODE_REQUIRES_METADATA: external dataType info needed",
    );
  }
...
function encodeNetworkMessage(networkMessage, opts) {
  if (!networkMessage || typeof networkMessage !== "object") {
    throw createError(
      "UADP_ENCODE_INVALID_INPUT: networkMessage must be an object",
    );
  }
  ...
  if (uadpFlags & 0x80 && extFlags1 & 0x80 && extFlags2 & 0x02) {
    throw createError(
      "UADP_ENCODE_NOT_YET_IMPLEMENTED: promotedFields (Phase 3 concern)",
    );
  }
  ...
  if (encoded.length > mtu && dsms.length > 0) {
    ...
    const messageSequenceNumber =
      networkMessage.groupHeader &&
      networkMessage.groupHeader.sequenceNumber != null
...
function decodeNetworkMessage(buffer, _opts) {
  ...
  if ((uadpFlags & 0x0f) !== UADP_VERSION) {
    throw createError(
      `UADP_DECODE_UNSUPPORTED_VERSION: expected ${UADP_VERSION}, got ${uadpFlags & 0x0f}`,
    );
  }
  ...
  if (extFlags2 & 0x02) {
    throw createError(
      "UADP_DECODE_NOT_YET_IMPLEMENTED: promotedFields decoding (Phase 3 concern)",
    );
  }
